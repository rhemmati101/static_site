...
def recursive_copy(source_path: str, destination_path: str, extension_from_paths: str) -> None:
    ...
    #assume source path input is a directory
    if not os.path.isdir(curr_source_path) or not os.path.isdir(curr_destination_path):
    ...
    for c in contents:
        if os.path.isfile(c):
            shutil.copy(c, curr_destination_path)
